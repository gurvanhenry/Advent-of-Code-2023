...
export function findFewestNumberOfCubesPerGame(
  input: string,
): { id: number; fewest: { blue: number; red: number; green: number } }[] {
  const games = buildGames(input);
  const fewestNumberOfCubesPerGame = games.map((x) => {
    return { id: x.id, fewest: convertRevealsToFewest(x.reveals) };
  });
  return fewestNumberOfCubesPerGame;
}
function convertRevealsToFewest(
  reveals: { blue: number; red: number; green: number }[],
): { blue: number; red: number; green: number } {
  return reveals.reduce(
    (prev, cur) => {
      return {
        blue: Math.max(prev.blue, cur.blue),
        red: Math.max(prev.red, cur.red),
        green: Math.max(prev.green, cur.green),
      };
    },
